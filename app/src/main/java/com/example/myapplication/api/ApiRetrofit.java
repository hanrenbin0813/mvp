...
import com.example.myapplication.gson.DoubleDefault0Adapter;
import com.example.myapplication.gson.IntegerDefault0Adapter;
import com.example.myapplication.gson.LongDefault0Adapter;
...
public class ApiRetrofit implements Interceptor {
    ...
    public final String BASE_SERVER_URL = "https://api.asilu.cc";
    ...
    public Cache cache() {
        ...
        if (isSdCardMounted) {
            // SD卡已挂载，可以继续操作
            cacheFile = new File(Environment.getExternalStorageDirectory().getAbsolutePath(), "httpCache");
            cache = new Cache(cacheFile, 1024 * 1024 * 100); //100Mb
            return cache;
        }else {

        }*/
        cacheFile=new File(App.getContext().getCacheDir().getAbsolutePath(),"httpCache");
